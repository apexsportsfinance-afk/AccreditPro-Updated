...
export default function BackgroundProgress() {
  ...
  return (
    <div className="fixed bottom-8 right-8 z-[100] w-80 bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl p-4 animate-in fade-in slide-in-from-bottom-4 duration-300">
      <div className="flex items-center justify-between mb-3">
        <h4 className="text-xs font-black uppercase tracking-widest text-slate-400">Processing Queue</h4>
        <span className="px-2 py-0.5 rounded-full bg-primary-500/10 text-primary-400 text-[10px] font-black">{queue.length + (processing ? 1 : 0)} TASKS</span>
      </div>
        ...
        {currentTask && (
          <div className="flex items-center gap-3 bg-slate-800/50 rounded-lg p-2.5 border border-white/5">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[10px] text-slate-500 font-medium uppercase tracking-tighter">Generating PDF & Sending Email...</p>
            </div>
          </div>
        )}
