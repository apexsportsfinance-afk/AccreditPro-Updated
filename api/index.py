    ...
    with pdfplumber.open(pdf_stream) as pdf:
        for page in pdf.pages:
            ...
            for line in lines:
                ...
                res_match = re.search(r"^(\d+)\s+([A-Za-z\s',\-]+)\s+(\d+)\s+(.*?)\s+([\d:\.]+|NT|DQ|NS|SCR|[\d:\.]+q)", line)
                if res_match:
                    place = int(res_match.group(1))
                    name = res_match.group(2).strip()
                    age = res_match.group(3).strip()
                    team = res_match.group(4).strip()
                    time_str = res_match.group(5).strip()
                    
                    try:
                        a = int(age)
                        if a <= 9: display_age_group = "8-9 Years"
                        elif a >= 16: display_age_group = "16 & Over"
                        else: display_age_group = f"{a} Year Olds"
                    except ValueError:
...
def extract_lines(file, filename):
    ...
    if ext == 'pdf':
        ...
        try:
            if ext == 'csv': df = pd.read_csv(file, header=None)
...
def parse_full_hytek(file, filename):
    ...
    for line in lines:
        ...
        # 1. Capture Event Headers with Round Detection
        ev_match = re.search(r"\(?Event\s+(\d+)\s+(Girls|Boys|Mixed|Men|Women|Open)\s+(.*)", line, re.I)
        if ev_match:
            current_event_code = ev_match.group(1)
            raw_gen = ev_match.group(2).lower()
            current_event_name = ev_match.group(3).strip()
            
            if "girl" in raw_gen or "women" in raw_gen or "female" in raw_gen:
                current_gender = "Female"
            elif "boy" in raw_gen or "men" in raw_gen or "male" in raw_gen:
                current_gender = "Male"
            else:
                current_gender = "Mixed"
                
            # Reset heat for new event
            current_heat = 1 
            # Detect Round
            header_content = line.lower()
            if re.search(r"prelim|semi|qualifying", header_content):
        ...
        # 2. Capture Result/Seed Rows
        row_match = re.search(r"(?:\s|^)(\d+)\s+([A-Z][A-Za-z\s',.\-]+?)\s+(\d{1,2})\s+(.*?)\s+(\d+[:\.]\d*[:\.]\d*|NT|NS|SCR|DQ|NP|[\d:\.]+q)", line, re.I)
        if row_match:
            place = int(row_match.group(1))
            ...
            results.append({
                ...
                "rank": place if is_result_sheet else None,
                "heat": current_heat,
                "lane": None if is_result_sheet else place,
                "round": current_round
            })
            continue
    return results
...
@app.route('/api/medal-upload', methods=['POST'])
def upload_files():
    ...
    return jsonify({"success": True, "count": len(all_results), "results": all_results})
